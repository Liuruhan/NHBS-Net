...
class AUNet_R16(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, learned_bilinear=False):
        super(AUNet_R16, self).__init__()
        self.n_channels=n_channels
        self.n_classes=n_classes

        self.down_block1 = UNet_residual_down_block(n_channels, 64, False)
...
if __name__ == "__main__":
    model = AUNet_R16(n_channels=3, n_classes=8, learned_bilinear=True)
    model.eval()
    image = torch.randn(1, 3, 256, 256)
    #image.flags.writeable = Trueimage
